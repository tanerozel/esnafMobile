...
ionViewWillEnter(){
  ...
  this.http.get( 'https://esnafimapi.azurewebsites.net/api/app/get_product_order/' + this.orderId ).toPromise()
  .then(data =>{         
    this.orderApproved = data;   

  }) 

  this.http.get( 'https://esnafimapi.azurewebsites.net/api/app/get_approved_orders/' + this.orderId ).toPromise()
  .then(data =>{         
    this.orders = data;
